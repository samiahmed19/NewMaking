...
app = Flask(__name__)
root = tk.Tk()
app.config['MONGO_URI'] = 'mongodb://localhost:27017/mydatabase_signup'
mongo = PyMongo(app)
bcrypt = Bcrypt(app)
CORS(app)
def start_tk():
...
def index():
    ...
    tk_thread = Thread(target=start_tk)
    tk_thread.start()
...
def signup():
    ...
    mongo.db.users.insert_one({
        ...
        'phoneNumber': data['phoneNumber']  # Include phone number in the user data
    })
...
def predict():
    ...
    def CNN_ALGO(data):
        ...
        print()
        print("##############################################################################")
        print("Tomorrow's ",quote," Closing Price Prediction by CNN: ", cnn_pred)
        print("CNN RMSE:", cnn_error)
    ...
    def stacking_algo(data):
        ...
        print("STACKING RMSE:", s_error)
        print("STACKING MAE:", s_mae)
        print("STACKING MSE:", s_mse)
        print("##############################################################################")
   ...
   #***************** VOTING SECTION ******************        
    def voting_algo(data):
        ...
        print("VOTING RMSE:",v_error)
        print("VOTING MAE:", v_mae)
        print("VOTING MSE:", v_mse)
        print("##############################################################################")
    ...
    try:
        ...
        if quote is None or not quote.strip():
            return jsonify({'error': 'Stock Symbol (Ticker) is required. Please Enter a Valid Ticker Symbol.'})
        else:
        ...
        df2 = pd.DataFrame({'Code': code_list})
        ...
        response_data = {
                        'quote': quote,
                        'linear_pred': round(linear_pred, 2).tolist(),
                        'gru_pred': np.round(gru_pred, 2).tolist(),
                        'lstm_pred': np.round(lstm_pred, 2).tolist(),
                        'bilstm_pred': round(bilstm_pred, 2).tolist(),
                        'cnn_pred': round(cnn_pred, 2).tolist(),
                        's_pred': round(s_pred, 2).tolist(),
                        'v_pred': round(v_pred, 2).tolist(),
                        ...
                        'error_linear': round(error_linear, 2),
                        'error_gru': round(error_gru, 2),
                        'error_lstm': np.round(error_lstm, 2).tolist(),
                        'error_bilstm': round(error_bilstm, 2),
                        'cnn_error': round(cnn_error, 2),
                        's_error': round(s_error, 2),
                        'v_error': round(v_error, 2),
                        ...
                        'lstm_img': f'http://localhost:5000/static/LSTM.png',
                        'bi_img': f'http://localhost:5000/static/BILSTM.png',
                        'cnn_img': f'http://localhost:5000/static/CNN.png',
                        'gru_img': f'http://localhost:5000/static/GRU.png',
                        'mlp_img': f'http://localhost:5000/static/MLP.png',
                        's_img': f'http://localhost:5000/static/stacking.png',
                        'v_img': f'http://localhost:5000/static/voting.png',
                        't_img': f'http://localhost:5000/static/Trends.png',
